...
function enforceBurst(identity: string, burstLimit: number): { ok: boolean; remaining: number; limit: number } {
  ...
  if (remaining <= 0) return { ok: false, remaining: 0, limit: burstLimit };

  map.set(key, used + 1);
  return { ok: true, remaining: Math.max(0, burstLimit - (used + 1)), limit: burstLimit };
}

async function resolveUserId(admin: any, req: any): Promise<string | null> {
...
export async function getAiUsageStatus(req: any): Promise<{
  ...
  // Anonymous users (best-effort caps)
  if (!userId) {
...
// NEW: increment usage after a successful AI call (best-effort).
// - Does not enforce quota (caller should have checked already)
// - Will still reset daily on first call of the day
// - For anonymous users, increments an in-memory counter
export async function incrementAiUsage(req: any, costUnits: number): Promise<{ ok: boolean }> {
  try {
    const supabaseUrl = process.env.SUPABASE_URL;
    const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
    ...
    if (!supabaseUrl || !serviceKey) {
      ...
      const map = getRateMap();
      const key = `AI_CAP:${today}:${identity}`;
      const used = map.get(key) || 0;
      map.set(key, used + Math.max(0, costUnits));
      return { ok: true };
    }
    ...
    const { data: profile, error: profileErr } = await admin
      .from('users')
      .select('id, generation_count, last_reset_date')
      .eq('id', userId)
      .maybeSingle();

    if (profileErr) console.error('Usage lookup error:', profileErr);

    let generationCount = profile?.generation_count ?? 0;
    let lastResetDateISO = profile?.last_reset_date ? new Date(profile.last_reset_date).toISOString() : new Date().toISOString();
    ...
    const newCount = generationCount + Math.max(0, costUnits);
    const { error: incErr } = await admin.from('users').update({ generation_count: newCount }).eq('id', userId);
    if (incErr) console.error('Usage increment error:', incErr);

    return { ok: true };
  } catch (e) {
    // Never throw from increment
    return { ok: true };
  }
}
...
export async function enforceAiUsage(req: any, costUnits: number): Promise<{
  ...
  if (lastKey !== today) {
    generationCount = 0;
    lastResetDateISO = new Date().toISOString();
    const { error: resetErr } = await admin.from('users').update({ generation_count: 0, last_reset_date: lastResetDateISO }).eq('id', userId);
    if (resetErr) console.error('Usage reset error:', resetErr);
  }
  ...
  if (!burst.ok) {
    return { ok: false, status: 429, error: 'Rate limit: too many requests per minute.', membership, burstRemaining: 0, burstLimit };
  }
  ...
  if (remaining < costUnits) {
    return { ok: false, status: 429, error: `Daily AI limit reached for your plan (${tier}).`, remaining, limit, membership: tier as any, burstRemaining: burst.remaining, burstLimit };
  }
  ...
  return { ok: true, remaining: Math.max(0, limit - newCount), limit, membership: tier as any, burstRemaining: burst.remaining, burstLimit };
}
