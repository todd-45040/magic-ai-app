...
async function resolveUserIdForBillingEvent(admin: any, params: {
  ...
  const stripeSubscriptionId = String(params.stripeSubscriptionId || '').trim() || null;
  if (stripeSubscriptionId) {
    ...
    const subUserId = String(data?.user_id || '').trim() || null;
    if (subUserId) return subUserId;
  }

  const stripeCustomerId = String(params.stripeCustomerId || '').trim() || null;
  if (stripeCustomerId) {
    ...
    const billingUserId = String(billingCustomer?.user_id || '').trim() || null;
    ...
    const directUserId = String(user?.id || '').trim() || null;
