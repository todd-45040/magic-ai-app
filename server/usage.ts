...
// Best-effort rollback for preflight-reserved usage when the upstream provider fails.
// This keeps general-user limits truthful: a failed Imagen/OpenAI call should not
// consume the user's daily AI count or monthly image balance. Admin calls are
// still never charged and this function no-ops for admins.
export async function refundAiUsage(
  req: any,
  costUnits: number,
  opts?: { tool?: string | null; reason?: string | null }
): Promise<void> {
  try {
    const units = Number.isFinite(costUnits) ? Math.max(0, Math.ceil(costUnits)) : 0;
    if (units <= 0) return;
    ...
    const { data: profileData } = await admin
      .from('users')
      .select('id, email, membership, is_admin, generation_count, trial_end_date, stripe_status, stripe_customer_id, stripe_subscription_id, quota_image_gen, quota_identify, quota_live_audio_minutes, quota_video_uploads')
      .eq('id', userId)
      .maybeSingle();

    const profile = withConfiguredAdminBypass(profileData, userId, userEmail) || buildAdminProfileFallback(userId, userEmail);
    if (isUnlimitedAdmin(profile)) return;

    const updates: Record<string, any> = {};
    updates.generation_count = Math.max(0, clampInt(profileData?.generation_count) - units);

    const toolKey = (typeof opts?.tool === 'string' ? opts.tool.trim() : '') as ToolKey | '';
    if (toolKey && (TOOL_POLICIES as any)[toolKey]) {
      const policy = (TOOL_POLICIES as any)[toolKey] as ToolPolicy;
      const col = policy.quotaColumn;
      updates[col] = clampInt((profileData as any)?.[col]) + units;
    }

    const { error: updateErr } = await admin.from('users').update(updates).eq('id', userId);
    if (updateErr) {
      console.error('Usage refund update error:', updateErr);
      return;
    }
    ...
    await safeLogUsageEvent({
      ...
      tool: opts?.tool ?? null,
      endpoint: req?.url ?? null,
      outcome: 'ERROR_UPSTREAM_REFUNDED',
      http_status: 502,
      error_code: opts?.reason ?? 'PROVIDER_ERROR',
      retryable: true,
      units,
      charged_units: 0,
      membership: normalizeTier(resolveEffectiveMembership(profile) as any),
      user_agent: req?.headers?.['user-agent'] || req?.headers?.['User-Agent'] || null,
      estimated_cost_usd: 0,
    });
  } catch (err) {
    console.error('refundAiUsage non-blocking error:', err);
  }
}
...
export async function enforceLiveMinutes(
  ...
  if (profileErr) {
    console.error('enforceLiveMinutes profile error:', profileErr);
