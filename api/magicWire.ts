...
function extractTag(block: string, tag: string): string | null {
  ...
  // This avoids "Invalid regular expression flags" on some builds/runtimes.
...
async function fetchRss(url: string, source: string): Promise<WireItem[]> {
  ...
  try {
    ...
    const parsed = blocks.slice(0, 20).map((b): WireItem => {
      const titleRaw = extractTag(b, "title") || "";
      const title = decodeHtml(stripTags(titleRaw)) || "Magic News";
...
export default async function handler(req: any, res: any) {
  try {
    ...
    // Fail-soft: empty is OK (don't 500 the page)
