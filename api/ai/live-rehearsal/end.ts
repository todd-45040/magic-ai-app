...
export default async function handler(req: any, res: any) {
  ...
  if (!result.ok) return jsonError(res, result.status || 400, { ok: false, error_code: result.error_code ?? 'LIVE_REHEARSAL_END_FAILED', message: result.message ?? 'Unable to end live rehearsal.', retryable: false });
