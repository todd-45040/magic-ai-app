...
type Membership = 'free' | 'trial' | 'amateur' | 'semi-pro' | 'professional' | 'expired';
...
export async function getAiUsageStatus(req: any): Promise<{
  ...
  const burstLimit = BURST_LIMITS[membership] ?? BURST_LIMITS.trial;
  const minuteKey = getMinuteKeyUTC();
  ...
  return { ok: true, membership, used: generationCount, limit, remaining, burstLimit, burstRemaining };
}
...
export async function enforceAiUsage(req: any, costUnits: number): Promise<{
  ...
  if (remaining < costUnits) {
    return {
      ok: false,
      status: 429,
      error: `Daily AI limit reached for your plan (${membership}).`,
      remaining,
      limit,
      membership,
