...
const Auth: React.FC<AuthProps> = ({ onBack, onLogin }) => {
  ...
  try {
    ...
    if (!sbUser?.id || !sbUser?.email) {
      throw new Error('Signed in, but no user session was detected. Please refresh and try again.');
    }

    // Build app user profile (safe defaults)
    let appUser: User = {
      email: sbUser.email,
      membership: 'trial',
      isAdmin: sbUser.email.toLowerCase() === ADMIN_EMAIL,
      generationCount: 0,
    ...
    // Ensure a user row exists (best-effort)
    await registerOrUpdateUser(appUser, sbUser.id);
    appUser = await checkAndUpdateUserTrialStatus(appUser, sbUser.id);

    // Immediate transition into the app (do not rely solely on the global auth listener)
    onLogin(appUser);
  } catch (err: any) {
...
const handleSignUpconst handleSignUp = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    setIsLoading(true);
