...
const VisualBrainstorm: React.FC<VisualBrainstormProps> = ({ onIdeaSaved }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        if (inputImageFile && inputImagePreview) {
            ...
            imageUrl = await editImageWithPrompt(base64Data, inputImageFile.type, prompt);
        } else {
            // Text-to-image flow
  ...
  const handleSaveImage = async () => {
    if (generatedImage) {
        await saveIdea('image', generatedImage);
        onIdeaSaved();
