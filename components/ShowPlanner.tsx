...
import type { Show, Task, Subtask, TaskPriority, Client, Finances, Expense, Performance } from '../types';
...
const TaskModal: React.FC<{
    ...
    const handleGeneratePatter = async () => {
        ...
        try {
            const patter = await generateResponse(title, IN_TASK_PATTER_SYSTEM_INSTRUCTION);
...
const ShowPlanner: React.FC<ShowPlannerProps> = ({ clients, onNavigateToAnalytics, initialShowId, initialTaskId }) => {
    ...
    // Show handlers
    const handleAddShow = (title: string, description?: string, clientId?: string) => {
        const newShows = addShow(title, description, clientId);
        setShows(newShows);
        setIsShowModalOpen(false);
    };
    const handleDeleteShow = (id: string) => {
        if (window.confirm('Are you sure you want to delete this entire show? This cannot be undone.')) {
            const newShows = deleteShow(id);
            setShows(newShows);
        }
    };
    const handleUpdateShow = (showId: string, updates: Partial<Show>) => {
        const newShows = updateShow(showId, updates);
        setShows(newShows);
        setSelectedShow(newShows.find(s => s.id === showId) || null);
    };
    
    // Task handlers
    const handleAddTask = (data: Omit<Task, 'id' | 'status' | 'createdAt'>) => {
        if (!selectedShow) return;
        const newShows = addTaskToShow(selectedShow.id, data);
        setShows(newShows);
        setSelectedShow(newShows.find(s => s.id === selectedShow.id) || null);
        setIsTaskModalOpen(false);
    };
    const handleUpdateTask = (data: Omit<Task, 'createdAt'>) => {
        if (!selectedShow) return;
        const newShows = updateTaskInShow(selectedShow.id, data.id, data);
        setShows(newShows);
        setSelectedShow(newShows.find(s => s.id === selectedShow.id) || null);
        setIsTaskModalOpen(false);
        setTaskToEdit(null);
    ...
    const handleToggleStatus = (task: Task) => {
        ...
        const newShows = updateTaskInShow(selectedShow.id, task.id, { status: newStatus });
        setShows(newShows);
        setSelectedShow(newShows.find(s => s.id === selectedShow.id) || null);
    };
    const handleDeleteTask = (id: string) => {
        ...
        if (window.confirm('Are you sure you want to delete this task?')) {
            const newShows = deleteTaskFromShow(selectedShow.id, id);
            setShows(newShows);
            setSelectedShow(newShows.find(s => s.id === selectedShow.id) || null);
        }
    };
    ...
    const handleToggleSubtask = (taskId: string, subtaskId: string) => {
        if (!selectedShow) return;
        const newShows = toggleSubtask(selectedShow.id, taskId, subtaskId);
        setShows(newShows);
        setSelectedShow(newShows.find(s => s.id === selectedShow.id) || null);
    };
    ...
    const ShowDetailView = () => {
        ...
        useEffect(() => {
            if (activeTab === 'history') {
                setPastPerformances(getPerformancesByShowId(selectedShow.id));
            }
        ...
        const handleAiSuggestTasks = async () => {
            ...
            try {
                ...
                const result = await generateStructuredResponse(prompt, AI_TASK_SUGGESTER_SYSTEM_INSTRUCTION, schema);
                
                if (result.tasks && Array.isArray(result.tasks)) {
                    const tasksData = result.tasks.map((title: string) => ({ title, priority: 'Medium' as const }));
                    const newShows = addTasksToShow(selectedShow.id, tasksData);
                    setShows(newShows);
                    setSelectedShow(newShows.find(s => s.id === selectedShow.id) || null);
                } else {
    ...
    return (
        <>
            {isTaskModalOpen && <TaskModal onClose={() => { setIsTaskModalOpen(false); setTaskToEdit(null); }} onSave={taskToEdit ? handleUpdateTask : handleAddTask} taskToEdit={taskToEdit} />}
...
const LivePerformanceModal: React.FC<{ show: Show; onClose: () => void; onEnd: (performanceId: string) => void }> = ({ show, onClose, onEnd }) => {
    ...
    useEffect(() => {
        const perf = startPerformance(show.id);
        setPerformance(perf);
