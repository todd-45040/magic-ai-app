...
const MarketingCampaign: React.FC<MarketingCampaignProps> = ({ user, onIdeaSaved, onNavigateToShowPlanner, onNavigate }) => {
    ...
    const [busyAction, setBusyAction] = useState<'copy' | 'save' | 'quick-export' | null>(null);
...
const showTimedNotice = (message: string, actionLabel?: string, action?: () => void, timeout = 2600) => {
    setActionNotice({ message, actionLabel, action });
    if (timeout > 0) {
        window.setTimeout(() => setActionNotice(null), timeout);
    }
};

const buildCampaignDocument = (titleOverride?: string) => {
    ...
    return `## ${titleOverride || `Marketing Campaign for: ${showTitle || '(untitled)'}`}${personaSuffix}\n\n${meta}\n\n---\n\n${activeResult || '(No generated output yet.)'}`;
};
...
const copyCampaignToClipboard = async () => {
    ...
    try {
        ...
        await navigator.clipboard.writeText(buildCampaignDocument());
        showTimedNotice('Campaign copied to clipboard.');
    } catch (err) {
        const msg = err instanceof Error ? err.message : 'Copy failed.';
        showTimedNotice(`Copy failed: ${msg}`, undefined, undefined, 3200);
    } finally {
        setBusyAction(null);
    }
};
...
const handleSave = async () => {
        ...
        try {
            await saveIdea({
                ...
                tags: ['marketing', 'campaign', ...(campaignStyle ? [campaignStyle] : []), ...selectedAudiences],
            });
            onIdeaSaved();
            setSaveStatus('saved');
            showTimedNotice('Campaign saved to Saved Ideas.');
            window.setTimeout(() => setSaveStatus('idle'), 2000);
        } catch (err) {
            const msg = err instanceof Error ? err.message : 'Unable to save campaign.';
            showTimedNotice(msg, undefined, undefined, 3200);
        } finally {
            setBusyAction(null);
        }
    };
    ...
    const handleBlueprintSave = async (label: 'Save as Template' | 'Save to Campaign Library' | 'Reuse Later' | 'Duplicate Campaign') => {
        if (!showTitle.trim()) {
            setShowTitleTouched(true);
            showTimedNotice('Add a show title before saving a blueprint.');
            return;
        }
        ...
        try {
            ...
            await saveIdea('text', content, title);
            onIdeaSaved();
            showTimedNotice('Blueprint saved to Saved Ideas.');
        } catch (err) {
            const msg = err instanceof Error ? err.message : 'Unable to save blueprint.';
            showTimedNotice(msg, undefined, undefined, 3200);
        } finally {
            setIsSavingBlueprint(false);
        }
    ...
    const handleSendToShowPlanner = async () => {
        ...
        try {
            ...
            const tasks = [
                { title: 'Marketing: Press Release', notes: activeResult, priority: 'Medium', status: 'To-Do' },
                { title: 'Marketing: Social Posts', notes: activeResult, priority: 'Medium', status: 'To-Do' },
                { title: 'Marketing: Email Campaign', notes: activeResult, priority: 'Medium', status: 'To-Do' },
                { title: 'Marketing: Poster / Flyer Copy', notes: activeResult, priority: 'Medium', status: 'To-Do' },
                { title: 'Marketing: Booking Pitch', notes: activeResult, priority: 'High', status: 'To-Do' },
            ];

            await addTasksToShow(created.id, tasks as any);

            showTimedNotice(`Saved to Show Planner: “${showTitle.trim()}”`, 'Open Show Planner', () => onNavigateToShowPlanner?.(created.id));
        } catch (e) {
    ...
    const handleQuickExportIdea = async (kind: 'Client Proposal' | 'Social Scheduler' | 'Booking Pitch Builder') => {
    if (!activeResult) return;
    setBusyAction('quick-export');
    ...
    try {
        const content = buildBlueprintContent(label);
        await saveIdea('text', content, `${label} — ${showTitle || 'Untitled'}`);
        onIdeaSaved();
        showTimedNotice(`${label} saved to Saved Ideas.`);
    } catch (err) {
        const msg = err instanceof Error ? err.message : `Unable to save ${label}.`;
        showTimedNotice(msg, undefined, undefined, 3200);
    } finally {
        setBusyAction(null);
    }
                        ...
                        <div className="sticky bottom-0 z-30 p-2.5 bg-slate-950/90 backdrop-blur-md flex flex-col gap-2 border-t border-slate-800 shadow-[0_-8px_24px_rgba(0,0,0,0.35)]">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                <button
                                    ...
                                    disabled={!activeResult || busyAction === 'quick-export'}
                                    className="flex items-center gap-2 px-3 py-1.5 text-sm bg-slate-700 hover:bg-slate-600 rounded-md text-slate-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                            ...
                            >
                                ...
                                <span>{busyAction === 'copy' ? 'Copying…' : 'Copy Campaign'}</span>
                            </button>
                            <CohesionActions
                                content={activeResult || ''}
                            ...
                            >
                             ...
                             <button
                                onClick={handleSave}
                                disabled={!activeResult || saveStatus === 'saved' || busyAction === 'save'}
                                className="flex items-center gap-2 px-3 py-1.5 text-sm bg-slate-700 hover:bg-slate-600 rounded-md text-slate-200 disabled:cursor-default transition-colors disabled:opacity-50"
                            >
                                {saveStatus === 'saved' ? (
                                    <>
                                    ...
                                    <>
                                        ...
                                        <span>{busyAction === 'save' ? 'Saving…' : 'Save Idea'}</span>
                                    </>
                                )}
                            </button>
