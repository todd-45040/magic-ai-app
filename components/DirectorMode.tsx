...
const DirectorMode: React.FC<DirectorModeProps> = ({ onIdeaSaved }) => {
    ...
    const handleAddToPlanner = () => {
        if (!showPlan) return;
        const newShows = addShow(showPlan.show_title, showPlan.show_description);
        ...
        showPlan.segments.forEach(segment => {
            segment.suggested_effects.forEach(effect => {
                const taskData = {
                    title: `${segment.title}: ${effect.type}`,
                    notes: effect.rationale,
                    priority: 'Medium' as const
                };
                showsWithTasks = addTaskToShow(newShow.id, taskData);
            });
        });
