...
interface BillingSettingsProps { user: User | null; onUpgrade: (selection: { tier:'amateur'|'professional'; billingCycle?: BillingCycle; founderRequested?: boolean; }) => void; }
...
const BillingSettings: React.FC<BillingSettingsProps> = ({ onUpgrade }) => {
  const [status, setStatus] = useState<BillingStatusPayload | null>(null); const [loading, setLoading] = useState(true); const [portalBusy, setPortalBusy] = useState(false); const [portalMessage, setPortalMessage] = useState(''); const [billingCycle, setBillingCycle] = useState<BillingCycle>('monthly'); const [founderRequested, setFounderRequested] = useState(false);
  useEffect(() => { let active = true; (async()=>{ try { const next = await fetchBillingStatus(); if (active) setStatus(next); } finally { if (active) setLoading(false); } })(); return ()=>{active=false;}; }, []);
  const currentPlanKey = status?.planKey || 'free';
  const founderLabel = useMemo(() => status?.founderProtected ? `${humanizePlan(status.founderLockedPlan)} at ${formatPriceCents(status.founderLockedPriceCents)}` : 'Standard pricing', [status]);
  const openPortal = async () => { setPortalBusy(true); setPortalMessage(''); try { const result = await createPortalSession(); if (result.url) window.location.href = result.url; else setPortalMessage(result.message || 'Billing portal is not available yet.'); } catch (e:any) { setPortalMessage(e?.message || 'Unable to open billing portal.'); } finally { setPortalBusy(false); } };
      ...
      {(['amateur','professional'] as const).map((tier) => {
        ...
        return <div key={tier} className={`rounded-2xl border p-5 ${tier==='amateur' ? 'border-purple-400/20 bg-purple-500/10' : 'border-amber-400/20 bg-amber-500/10'}`}>
          <div className="flex items-center justify-between gap-3">
            <div><div className="flex items-center gap-2"><p className="text-sm font-semibold">{tier === 'amateur' ? 'Amateur' : 'Professional'}</p>{current ? <span className="rounded-full border border-white/20 px-2 py-0.5 text-[10px] uppercase">Current</span> : <span className="rounded-full border border-white/20 px-2 py-0.5 text-[10px] uppercase">{badge}</span>}</div><p className="mt-1 text-sm text-white/65">{tier==='amateur' ? 'Creative tools, saved workspaces, and broader practice access.' : 'Full rehearsal, performance, and business operating system access.'}</p></div>
          ...
          <button onClick={() => onUpgrade({ tier, billingCycle, founderRequested })} disabled={loading || current} className={`mt-4 inline-flex w-full items-center justify-center rounded-xl px-4 py-3 text-sm font-bold transition ${tier==='amateur' ? 'bg-purple-600 text-white hover:bg-purple-700' : 'bg-amber-500 text-slate-950 hover:bg-amber-400'} disabled:cursor-not-allowed disabled:opacity-50`}>{current ? 'Current plan' : `Upgrade to ${tier === 'amateur' ? 'Amateur' : 'Professional'}`}</button>
        </div>;
      })}
    </div>
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-white">
