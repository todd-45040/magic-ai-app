...
const ClientProposals: React.FC<ClientProposalsProps> = ({ initialId, onBack }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!alive) return;
        setNotice(String(e?.message ?? 'Failed to load proposals'));
      } finally {
    ...
    <div className="w-full h-full flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-slate-200">
          ...
          <h2 className="text-lg font-semibold">Client Proposals</h2>
        </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-3 min-h-0">
        ...
        <div className="lg:col-span-1 bg-slate-900/30 border border-slate-800 rounded-lg p-3 min-h-0 flex flex-col">
          ...
          {loading ? (
            ...
            <div className="text-sm text-slate-400">
              No proposals yet. Create one from the Marketing Campaign Generator.
            </div>
          ) : (
        ...
        <div className="lg:col-span-2 bg-slate-900/30 border border-slate-800 rounded-lg p-3 min-h-0 flex flex-col">
          {!selected ? (
            <div className="text-sm text-slate-400">
              Select a proposal on the left to view it here.
            </div>
          ) : (
            <>
