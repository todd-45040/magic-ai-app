...
// --- Reliability helpers (quota + timeout + never-stuck loading) ---
...
function normalizeAiError(err: unknown): NormalizedAiError {
  ...
  if (isQuota) {
    return {
      userMessage:
        'AI quota or billing limit reached. Please confirm your Google AI billing/quota, then try again in a moment.',
      isQuota: true,
      isTimeout: false,
    };
  }
...
const PropChecklists: React.FC<PropChecklistsProps> = ({ onIdeaSaved }) => {
  ...
  // Prevent late-resolving promises from updating state after unmount / after a newer request.
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Invalidate any in-flight request so its resolution can't affect UI state.
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Only update state if this is still the latest request and we're mounted.
  ...
  const handleClearAll = () => {
    // Invalidate any in-flight request and reset UI state.
    requestIdRef.current += 1;
    setIsLoading(false);
    setError(null);
    setChecklist(null);
