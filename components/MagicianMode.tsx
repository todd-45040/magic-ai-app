...
import { MAGICIAN_SYSTEM_INSTRUCTION, MAGICIAN_PROMPTS, publications, clubs, conventions, AMATEUR_FEATURES, PROFESSIONAL_FEATURES, ADMIN_EMAIL, MAGICIAN_CHAT_TOOLS } from '../constants';
...
// --- Helper components for inline forms ---
// Moved outside MagicianMode to prevent re-rendering issues that cause input focus loss.
...
}> = ({ prompts, user, hasAmateurAccess, hasProfessionalAccess, onPromptClick }) => {
  return (
    <div className="flex-1 flex flex-col justify-center items-center text-center animate-fade-in p-4">
      ...
      <div className="w-full max-w-4xl grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {prompts.filter(p => user.isAdmin || p.title !== 'Member Management').map((p) => {
            ...
            const isLocked = (isAmateurFeature && !hasAmateurAccess) || (isProfessionalFeature && !hasProfessionalAccess);
...
// --- Magician Mode Child Components (moved outside main component) ---
...
const MagicianMode: React.FC<MagicianModeProps> = ({ onBack, user, onUpgrade, onLogout }) => {
  ...
  // New state for performance analytics navigation
  const [viewingPerformanceId, setViewingPerformanceId] = useState<string | null>(null);

  // State for deep-linking from Global Search
  ...
  const hasAmateurAccess = user.membership === 'amateur' || user.membership === 'professional' || isTrialActive;
  const hasProfessionalAccess = user.membership === 'professional' || isTrialActive;
  ...
  const handleSend = async (prompt?: string) => {
    ...
    // Build API history from UI messages
    ...
    try {
        ...
        if (functionCalls && functionCalls.length > 0) {
            ...
            if (functionCall.name === 'createTask') {
                ...
                const show = findShowByTitle(showName);
                if (show) {
                    addTaskToShow(show.id, { title: taskTitle, priority: (priority as TaskPriority) || 'Medium' });
  ...
  const handlePromptClick = (prompt: PredefinedPrompt) => {
    ...
    if ((isAmateurFeature && !hasAmateurAccess) || (isProfessionalFeature && !hasProfessionalAccess)) {
  ...
  const handleSaveIdea = (text: string, index: number) => {
    saveIdea('text', text);
  ...
  const handleIdentifyClick = async () => {
    ...
    try {
        const result = await identifyTrickFromImage(base64Data, mimeType);
  ...
  const handleDeepLink = (view: MagicianView, primaryId: string, secondaryId?: string) => {
    // Reset any previous deep-link state
  ...
  const renderContent = () => {
    switch(activeView) {
        // FIX: Pass the correct `handleShowsUpdate` and `handlePromptClick` functions to the Dashboard component.
        ...
        case 'video-rehearsal': return <VideoRehearsal onIdeaSaved={() => handleIdeaSaved('Video analysis saved!')} />;
        case 'visual-brainstorm': return <VisualBrainstorm onIdeaSaved={() => handleIdeaSaved('Image idea saved!')} />;
        ...
        case 'show-planner': return <ShowPlanner clients={clients} onNavigateToAnalytics={handleNavigateToAnalytics} initialShowId={initialShowId} initialTaskId={initialTaskId} />;
        ...
        case 'patter-engine': return <PatterEngine onIdeaSaved={() => handleIdeaSaved('Patter ideas saved!')} />;
        case 'marketing-campaign': return <MarketingCampaign onIdeaSaved={() => handleIdeaSaved('Marketing campaign ideas saved!')} />;
        case 'contract-generator': return <ContractGenerator onIdeaSaved={() => handleIdeaSaved('Contract saved!')} />;
        case 'assistant-studio': return <AssistantStudio onIdeaSaved={() => handleIdeaSaved('Assistant idea saved!')} />;
        case 'director-mode': return <DirectorMode onIdeaSaved={() => handleIdeaSaved('Show Plan saved!')} />;
        case 'illusion-blueprint': return <IllusionBlueprint onIdeaSaved={() => handleIdeaSaved('Illusion Blueprint saved!')} />;
        case 'magic-theory-tutor': return <MagicTheoryTutor />;
        case 'magic-dictionary': return <MagicDictionary />;
        case 'persona-simulator': return <PersonaSimulator onIdeaSaved={() => handleIdeaSaved('Persona simulation saved!')} />;
        case 'gospel-magic-assistant': return <GospelMagicAssistant onIdeaSaved={() => handleIdeaSaved('Gospel routine idea saved!')} />;
        case 'mentalism-assistant': return <MentalismAssistant onIdeaSaved={() => handleIdeaSaved('Mentalism idea saved!')} />;
  ...
  return (
    <div className="relative flex flex-col h-full rounded-lg border border-slate-800 shadow-2xl shadow-purple-900/20 overflow-hidden">
      ...
      <header className="flex items-center p-4 border-b border-slate-800">
        ...
        <RabbitIcon className="w-8 h-8 text-purple-400 mr-3" />
        <h1 className="font-cinzel text-2xl font-bold text-white">Magicians' AI Wizard</h1>
