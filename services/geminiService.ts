...
async function postJson<T>(
  ...
  const timeoutMs = options?.timeoutMs ?? 45000; // keep under common serverless proxy timeouts
  ...
  const msg = lastErr?.name === 'AbortError'
    ? `Request timed out (${Math.round((options?.timeoutMs ?? 45000) / 1000)}s)`
...
export const generateResponse = async (
  ...
  try {
    const result = await postJson<any>('/api/generate', body, currentUser, options?.extraHeaders, { timeoutMs: 45000, retries: 1 });
