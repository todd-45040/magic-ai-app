...
export const registerOrUpdateUser = async (user: User, uid: string): Promise<void> => {
  try {
    ...
    const row: any = {
      ...
      signup_partner_source: requestedSource || 'direct',
      requested_trial_days: membership === 'trial' ? requestedTrialDays : null,
      partner_detail_value: requestedIbmRing || null,
      partner_detail_value: requestedSamAssembly || null,
      partner_partner_source: partnerContext.partnerSource || null,
      partner_partner_campaign: partnerContext.partnerCampaign || null,
