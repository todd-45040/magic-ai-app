...
export type BillingStatusPayload = {
  compAccessGrant: { id:string; planKey:BillingPlanKey; startsAt:string; endsAt:string; status:string; source:string|null; grantedBy:string|null; grantReason:string|null; email:string|null; userId:string|null; } | null;
  ok:true; planKey:BillingPlanKey; billingStatus:string; accessState:string; renewalDate:string|null; cancelAtPeriodEnd:boolean; founderProtected:boolean; founderLockedPlan:BillingPlanKey|null; founderLockedPriceCents:number|null; usagePeriodStart:string|null; usagePeriodEnd:string|null; upgradeTargets:BillingPlanKey[]; stripeConfigured:boolean; billingCustomerExists:boolean; stripeCustomerIdPresent:boolean; currentBillingCycle: BillingCycle; currentPriceId: string | null; source:'database'|'fallback'|'stripe_live'|'comp_access'; billingTruth: BillingTruthDebug; recentBillingEvents: BillingEventSnapshot[]; validationChecks: BillingValidationChecks; validationGuide: BillingValidationGuide; billingReadiness:{ expectedWebhookPath:string; expectedWebhookUrl:string; missingEnvKeys:string[]; configuredPriceKeys:string[]; missingPriceKeys:string[]; hasPublishableKey:boolean; hasWebhookSecret:boolean; hasServerSecretKey:boolean; };
};
